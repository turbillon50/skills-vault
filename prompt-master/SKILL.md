...
- El Memory Block NO se inventa del chat: primero se arma desde el Brain (brain_query / mci_memoria) y el contexto del proyecto; lo del chat solo complementa.
- Uso principal: prompts de handoff a Claude Code (ver sync-protocol) y prompts para modelos que rutea V (deepseek-chat, minimax, gemini, llama-3.1-8b), que necesitan instrucciones mas cerradas que Claude.
- Regla de colocacion: el objetivo es que salga bien a la primera, no gastar menos.
